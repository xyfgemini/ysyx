...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulsu  , R, R(rd) = ((sword_t)src1 * src2) >> 32);
  // INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulu   , R, R(rd) = (src1 * src2) >> 32);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4 ; s->dnpc = src1 + imm ; if(rd == 0 && src1 == 1 && imm == 0) {inst_ret = true;} else {inst_call = true;});
