...
static bool inst_call = false;
static bool inst_ret = false;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> src2); //logical(default by 0) 
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (sword_t)src1 >> src2); //arithmatic(look at sign bit) 
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = (int32_t)(int16_t)Mr(src1 + imm,2)); //sign extended to 32 
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm,2)); //zero extended to 32
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = (int32_t)(int8_t)Mr(src1 + imm,1)); //sign extended to 32 
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm,1));//zero extended to 32
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4 ; s->dnpc = src1 + imm ; \
                                                                if(rd == 0 && src1 == 1 && imm == 0) {inst_ret = true;} \
                                                                 else {inst_call = true;});
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4 ; s->dnpc = s->pc + imm; inst_call = true);
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div     , R, if((sword_t)src2 == -1){ R(rd) = (sword_t)0x80000000;} \
                                                                else if((sword_t)src2 == 0){ R(rd) = -1;} \
                                                                else { R(rd) = (sword_t)src1 / (sword_t)src2;});

  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2 == 0) ? (uint32_t)0xffffffff : src1 / src2);

  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if((sword_t)src2 == 0) {R(rd) = src1;} \
                                                                else if ((sword_t)src1 == (sword_t)0x80000000 && (sword_t)src2 == -1) { R(rd) = 0;} \
                                                                else { R(rd) = (sword_t)src1 % (sword_t)src2;});
  ...
  // #ifdef CONFIG_FTRACE
  //   ftrace_display(inst_call,inst_ret,s->pc,s->dnpc);
  //   ftrace_free();
  // #endif
