...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc+4; s->dnpc = src1 + imm; cpu.pc = s->dnpc);

  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));

  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, cpu.pc = (src1 == src2) ? (s->pc + imm) : (s->pc + 4));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, cpu.pc = (src1 != src2) ?  (s->pc + imm) : (s->pc + 4));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, cpu.pc = (src1 < src2) ? (s->pc + imm) : (s->pc + 4));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, cpu.pc = (src1 >= src2) ? (s->pc + imm) : (s->pc + 4));
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, cpu.pc = (src1 < src2) ? (s->pc + imm) : (s->pc + 4));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, cpu.pc = (src1 >= src2) ? (s->pc + imm) : (s->pc + 4));

  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm);
  
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm ; cpu.pc = s->dnpc);
